...
const DataVisualizer = () => {
  const [tables, setTables] = useState(['inventory', 'sales', 'purchases']);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/${selectedTable}`);
  ...
  return (
    <div className="flex flex-col items-center p-4">
      <div className="w-full max-w-md bg-secondary text-white rounded-lg shadow-lg p-4">
        <div className="mb-4">
          <label className="block text-gray-300 mb-2">Select Table:</label>
          <select
            value={selectedTable}
